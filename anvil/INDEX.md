...
| [java-spring-coder](skills/java-spring-coder/SKILL.md) | 1.13 | Java Spring Boot 4-Tier 코드 생성·단위 테스트 구현 (v1.13: Airbridge 아웃박스 패턴 + 재전송 배치 흡수 / v1.12: rubric 복붙 오류 수정 / v1.11: Hibernate Session 오염 방지 3연타 + @ConditionalOnBean 회피 + 캐시 pub-sub + 어노테이션 인터셉터) | **회귀 통과 95/100** (2026-07-09 --skip-baseline PASS, TC 4/4. 하네스 커버리지 갭 있음) | 일반 | 95/100 (v1.11 회귀) | `skills/java-spring-coder/` |
| [self-code-reviewer](skills/self-code-reviewer/SKILL.md) | 2.0 | **공통 룰 + 관점 리뷰 오케스트레이션 안내** (v2.0 슬림화: KISA→secure-coding / WebClient·싱글톤·Soft-delete UNIQUE→performance / FQCN·Bean Qualifier·구현체 모듈→architecture 완전 이관. 잔존: @Profile·광범위 catch·Session 오염·입력 검증·i18n·임시 로그) | **회귀 통과 87.25/100** (2026-07-09 --skip-baseline PASS, 스코프 침범 0건. 하네스 리팩터 시급) | 일반 | 87.25/100 (v2.0 회귀) | `skills/self-code-reviewer/` |
...
| [java-performance-reviewer](skills/java-performance-reviewer/SKILL.md) | 0.2 | **성능 관점 전용 리뷰** — N+1·JPA·인덱스 + 캐시 계층 + 트랜잭션·비동기·스레드 풀 + 리소스 누수·GC + **PERF-OPS(HikariCP right-size·graceful shutdown·startup probe)** + 로깅 스팸 | **테스트 대기** (하네스 3종 작성 완료, `/eval-harness` 미실행) | 일반 | - | `skills/java-performance-reviewer/` |
...
| [java-business-logic-reviewer](skills/java-business-logic-reviewer/SKILL.md) | 0.1 | **비즈니스 로직·요건 준수 관점 전용 리뷰** — PRD 수용 기준·TDD 설계 vs 실제 구현 코드 정합성 검증 + 도메인 불변식·엣지 케이스 사각지대 검출 (문서↔코드, prd-plan-designer 문서↔문서와 층위 분리). self-code-reviewer 세분화 4번째 관점(2026-07-09) 신설 | **테스트 대기** (하네스 3종 작성 완료, `/eval-harness` 미실행) | 일반 | - | `skills/java-business-logic-reviewer/` |
...
| [tolgee](skills/tolgee/SKILL.md) | 0.2 | Tolgee i18n 콘솔과 properties 파일 동기화 (v0.2: 4파일 동시 갱신 강제 + en 카피 품질 가드 + Java 코드 Locale 함정 안내) | **테스트 대기** (하네스 3종 작성 완료, `/eval-harness` 미실행). 6/24 FAIL → 7/8 PASS 사례 축적 | 일반 | - | `skills/tolgee/` |
...
| java-spring-coder | skill | 1.12 | **1.13** | **재배포 필요** (2026-07-09 v1.13: Airbridge 아웃박스 패턴 + 재전송 배치) |
...
| java-performance-reviewer | skill | 0.1 | **0.2** | **재배포 필요** (2026-07-09 v0.2: PERF-OPS 신규 유형 — HikariCP right-size·graceful shutdown·startup probe) |
...
| java-composite-reviewer | agent | 0.1 | 0.1 | **동기화** (2026-07-24 `anvil/`·`forge/common/pasta-rules/`·`forge/common/negative-tc-catalog.md` 미리매핑 참조 정정 + negative-tc-catalog.md `.claude/shared/`에 신규 배포. 2026-07-09 신규 배포: 4개 관점 리뷰 오케스트레이터) |
...
| java-layered-unit-testing | skill | 1.4 | **1.5** | **재배포 필요** (2026-07-09 v1.5: Testcontainers 통합 테스트 가이드라인 5개 트리거 — #633 후속 반영) |
| java-business-logic-reviewer | skill | - | **0.1** | **신규 배포 대기** (2026-07-09 신설: PRD/TDD ↔ 실제 구현 코드 정합성 검증, 도메인 불변식·엣지 케이스 사각지대) |
