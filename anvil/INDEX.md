...
|------|------|------|------|---------|------------|------|
| [prd-designer](skills/prd-designer/SKILL.md) | 1.2 | PRD 기획 요구사항 문서 작성 (v1.2: Phase 1.5 현황 파악 + 멀티턴·풀패키지 결정 트리 + 확인vs가정 분리) | **회귀 통과 97/100** (2026-07-29, TC 3/3 EXCELLENT. v1.2 신규 룰 5/6 커버) | 일반 | 97/100 (v1.2) | `skills/prd-designer/` |
| [tdd-designer](skills/tdd-designer/SKILL.md) | 1.4 | TDD 기술 설계 문서 작성 (v1.4: Phase 0.5 현황 파악 + 결정 트리 + 자기 검증 v2.0) | **회귀 통과 96.67/100** (2026-07-29, TC 3/3. v1.4 신규 룰 4건 모두 통과) | 일반 | 96.67/100 (v1.4) | `skills/tdd-designer/` |
...
| [pr-feedback-resolver](skills/pr-feedback-resolver/SKILL.md) | 1.5 | PR 피드백 수집·수정·push·답글 (v1.5: 자기 검증 v2.0 — PR 컨텍스트 현황 파악 + 봇/사람 분기 근거 기록) | **회귀 통과 97/100** (2026-07-29, TC 3/3. 이전 94.5 대비 +2.5) | 일반 | 97/100 (v1.5) | `skills/pr-feedback-resolver/` |
| [admin-prd-plan-designer](skills/admin-prd-plan-designer/SKILL.md) | 1.1 | Admin 모듈 전용 PRD/TDD/HYBRID 계획 문서 작성 (v1.1: Phase 0.3 admin 모듈 현황 파악 + 멀티턴/풀패키지 결정 트리) | **회귀 통과 93/100** (2026-07-29, TC 3/3. 이전 88 대비 +5. AUTO FAIL 5/5) | 일반 | 93/100 (v1.1) | `skills/admin-prd-plan-designer/` |
| [admin-thymeleaf-ui](skills/admin-thymeleaf-ui/SKILL.md) | 1.2 | Admin Thymeleaf SSR 화면 구현/수정 (v1.2: 자기 검증 v2.0 — 기존 패턴 사전 스캔 + 영향도 grep 확인) | **회귀 통과 99/100** (2026-07-29, TC 3/3. 이전 95.3 대비 +3.7. AUTO FAIL 6/6) | 일반 | 99/100 (v1.2) | `skills/admin-thymeleaf-ui/` |
| [api-inventory-generator](skills/api-inventory-generator/SKILL.md) | 1.1 | @RestController 스캔 → API 전체 목록 자동 생성 (v1.1: 자기 검증 v2.0 — 스캔 범위 명시 + 추정 vs 확정 분리) | **회귀 통과 98.3/100** (2026-07-29, TC 3/3. 이전 89 대비 +9.3, 최대 개선 폭) | 일반 | 98.3/100 (v1.1) | `skills/api-inventory-generator/` |
...
| [chaos-test-planner](skills/chaos-test-planner/SKILL.md) | 1.2 | Chaos Monkey 장애 테스트 검토·계획·커맨드 생성 (v1.2: Phase 1.5 환경 현황 파악 + 자기 검증 6항목) | **회귀 통과 96.67/100** (2026-07-29, TC 3/3. 이전 93 대비 +3.67. AUTO FAIL 4/4) | 일반 | 96.67/100 (v1.2) | `skills/chaos-test-planner/` |
| [sq-tone-writer](skills/sq-tone-writer/SKILL.md) | 1.4 | 사용자 말투로 슬랙/PR/문서/리뷰 답글 작성 (v1.4: 자기 검증 v2.0 — 수신자 파악 근거 + 봇/사람 분류 근거 + 가정 답변 표기) | **회귀 통과 96.8/100** (2026-07-29, TC 5/5. 이전 95.5 대비 +1.3. AUTO FAIL 4/4) | 일반 | 96.8/100 (v1.4) | `skills/sq-tone-writer/` |
| [jira-bug-root-cause](skills/jira-bug-root-cause/SKILL.md) | 1.1 | Jira 버그 원인 규명 + QA 친화 코멘트 작성 (v1.1: 자기 검증 v2.0 — 확인 vs 가정 분리 + 운영 트래픽 출처 명시 + 재현 조건 검증) | **회귀 통과 98.3/100** (2026-07-29, TC 3/3. 이전 91 대비 +7.3. AUTO FAIL 4/4) | 일반 | 98.3/100 (v1.1) | `skills/jira-bug-root-cause/` |
...
| [sq-today-reviewer](skills/sq-today-reviewer/SKILL.md) | 0.3 | 하루 Claude 작업 총체 리뷰 → 놓친 학습·공부 주제(실무/CS이론 2트랙)·반복 실수·소양·내일 액션 6섹션 성장 리포트 | **회귀 통과 95.5/100** (2026-07-29 첫 회귀, TC 4/4. AUTO FAIL 6/6, v0.3 2트랙 분리 룰 정합) | 일반 | 95.5/100 (v0.3) | `skills/sq-today-reviewer/` |
...
| [error-log-report](skills/error-log-report/SKILL.md) | 0.2 | **구간 에러 로그 공식 보고서** — GCP Cloud Logging ERROR 수집 → 유형 분류 → 노이즈 카탈로그 대조 신규/기존 판정 → 딥링크 포함 7섹션 보고서 (moneyball 역수입·일반화. 로그 건수 vs 실패 이벤트 수 분리, 마스킹 원인 예외 추적, 후처리 실패 vs 원본 유실 구분. v0.2: 집계 버킷 임의 세분화 금지) | **실전 배치 가능 — 6축 전부 충족** (2026-07-29, TC 5/5, AUTO FAIL 10/10 방어, Baseline +49.4, **일관성 PASS** TC-1 `--repeat 3` 97/97/95 편차 2점) | 일반 | 94.6/100 (v0.1 평가) | `skills/error-log-report/` |
| [batch-schedule-audit](skills/batch-schedule-audit/SKILL.md) | 0.3 | **배치 스케줄 감사** — Cloud Run Jobs + Scheduler 전수 조사 → 자원별(PRIMARY/REPLICA/AGGREGATE/외부) 무게 산정 → KST 타임라인 cron 충돌 분석 → 재배치 제안 + 신규 배치 평가 (moneyball 역수입·일반화. `scripts/collect.sh` 동반. v0.2: 합성 단일 무게 점수 금지 / v0.3: 고빈도 잡 정시 offset 재배치) | **실전 배치 가능 — 6축 전부 충족** (2026-07-29, TC 5/5, Baseline +37.8, **일관성 PASS** TC-1 `--repeat 3` 97/95/96 편차 2점 + 01:00 충돌 3건 판정 3회 일치. TC-4 v0.2 재검증 78→93) | 일반 | 93.2/100 (v0.1 평가) | `skills/batch-schedule-audit/` |
...
| error-log-report | skill | (moneyball 원본) | 0.1 | **2026-07-29 신규 역수입** — moneyball 자체 개발 스킬을 forge로 흡수·일반화. moneyball 측 원본은 그대로 유지(미교체). forge 0.1은 프로젝트 파라미터를 Phase 0으로 분리하고 노이즈 카탈로그를 `references/noise-catalog-moneyball.md`로 옮긴 버전. 평가 통과 후 재배포 검토 |
| batch-schedule-audit | skill | (moneyball 원본) | 0.1 | **2026-07-29 신규 역수입** — 동일. forge 0.1은 토폴로지를 `references/topology-moneyball.md`로 분리 + `collect.sh` 인자 필수화. moneyball 측 원본 유지(미교체) |

> **2026-07-15 패리티 재배포**로 7/7 배포의 두 부채 해소: ①커맨드 8종 통째 누락(`/git-commit` 등 미노출) ②7/9 리뷰 세분화 사이클 미전파. rules는 전체 세트가 아닌 **범용 Java Spring 컨벤션(01~09, 17~19)** 만 배포 — pasta admin 전용(13~16)·worktree(10)·multipart(12)는 moneyball 무관으로 제외. 이로 인해 java-architecture-reviewer의 admin 컨벤션 대조는 degrade(해당 규칙 부재 시 건너뜀)되나 moneyball은 admin 모듈이 없어 영향 없음. 기존 `.claude/skills/batch-schedule-audit/`·`error-log-report/`는 당시 forge 비소유(moneyball 자체 컴포넌트)로 배포 대상 제외·유지 → **2026-07-29 forge로 역수입 완료**(위 표 참조).

> **2026-07-29 moneyball 역수입 사이클**: moneyball 자체 개발 스킬 2건(`error-log-report`, `batch-schedule-audit`)을 forge로 흡수. moneyball `.claude/`가 `.gitignore:53`에 걸려 있어 두 스킬은 git 히스토리가 없었다(유실 위험). 본문은 일반화하고 프로젝트 특정 값은 `references/{noise-catalog,topology}-moneyball.md`로 분리. 동시에 확인된 **moneyball 동기화 부채**: java-spring-coder 1.12(forge 1.13), java-performance-reviewer 0.1(forge 0.2, PERF-OPS 없음), java-composite-reviewer 0.1(forge 0.2, 5관점 미확장), self-code-reviewer 루브릭 v2.0 리팩터 미반영, **java-business-logic-reviewer·java-layered-unit-testing 미배포**, tolgee `references/` 누락 → 7/9 사이클3 + 7/29 사이클4 미전파. `/forge-deploy --sync` 필요.
