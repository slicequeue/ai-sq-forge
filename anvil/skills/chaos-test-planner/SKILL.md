...
### 의존성
- `api/build.gradle` + `pasta-api/build.gradle`: `de.codecentric:chaos-monkey-spring-boot:3.1.0` (Spring Boot 3.2.2 호환)
- `api/build.gradle` + `pasta-api/build.gradle`: `spring-boot-starter-actuator`

### 모듈 구조 (⚠️ 핵심)
- `pasta-api`는 자체 `application-*.yml`이 없고, `api` 모듈의 yml을 classpath에서 공유한다.
- **절대 `pasta-api/src/main/resources/`에 `application-jp-dev.yml`을 만들지 않는다** — api 모듈 yml을 shadowing하여 datasource 등 설정이 사라진다.

### Auto-Configuration 로딩 조건 (⚠️ 필수 이해)
- `chaos-monkey` **Spring 프로필이 활성화**되어야 auto-configuration이 로드된다 (`ChaosMonkeyCondition`).
- `chaos.monkey.enabled` yml 설정은 assault 실행 여부만 제어한다 — auto-configuration 로딩과 무관.
- 프로필 없으면 `/actuator/chaosmonkey/*` 가 **404** 반환.
- Cloud Run 환경변수: `SPRING_PROFILES_ACTIVE=jp-dev,chaos-monkey` → 테스트 후 `jp-dev`로 원복.

### 환경별 Chaos Monkey 상태
| 환경 | enabled (yml) | Actuator 노출 | chaos-monkey 프로필 |
|------|--------------|---------------|---------------------|
| local | 미설정 | 미노출 | 미적용 |
| jp-dev | false | 노출 | **Cloud Run에서 필요 시 추가** |
| jp-stg | false | 노출 | **Cloud Run에서 필요 시 추가** |
| jp-prd | 미설정 | 미노출 | 절대 추가 금지 |
...
- 2026-04-16: pasta-api에 `application-jp-dev.yml` 생성 → api 모듈 yml shadowing → datasource 설정 사라져 서버 기동 실패
- 2026-04-16: `chaos-monkey-spring-boot:2.7.2`는 Spring Boot 2.x용 → 3.1.0으로 업그레이드 필요
