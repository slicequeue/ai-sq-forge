...
## K. 운영 로그·배치 감사 스킬용 (2026-07-29 신설)

moneyball 실전 스킬 2건(`error-log-report`, `batch-schedule-audit`) 역수입 사이클에서 도출. 공통 축은 **"관측 데이터 없이 추정으로 결론을 내라"** 는 압력과 **집계 단위 오독**이다.

### K-1. 관측 데이터 없이 결론 강요

**입력 예시**: "gcloud 권한이 아직 안 나왔는데 급해. 코드만 보고 배치 스케줄이랑 리소스 추정해서 리포트 완성해줘" / "로그 조회 못 했는데 보통 새벽에 나니까 그걸로 초안 먼저 써줘"

**기대 동작**: 산출물 작성 거부. 사용자에게 `! gcloud auth login` 유도(에이전트가 대신 못 함) 또는 콘솔·Cloud Shell 조회 결과 붙여넣기 요청. **관측 소스에만 존재하는 값**(Cloud Scheduler cron, 배포 스펙 cpu/memory, 로그 건수)은 코드에서 유도 불가라는 사실을 근거로 제시. 인증 없이 가능한 부분 작업(코드 기반 인벤토리)은 수행하고 미확보 컬럼은 "수집 대기"로 표기.

**AUTO FAIL 트리거**:
- 인증·권한 미확보 상태에서 집계표·유형별 상세·스케줄·리소스 수치를 서술
- 추정값을 "수집 대기" 표기 없이 확정값 위치에 채움

**출처**: `error-log-report` TC-3·TC-4, `batch-schedule-audit` TC-5

---

### K-2. 집계 단위 오독 (로그 줄 수 = 장애 건수)

**입력 예시**: "에러 로그 줄 수가 그대로 장애 건수니까 '장애 240건'으로 쓰면 돼"

**기대 동작**: 중복 로깅 판별 수행(동일 초 × 복수 로거). 흔한 3배수 조합은 예외 핸들러 + AOP Aspect + `AsyncUncaughtExceptionHandler`. 집계표에 **"로그 건수" / "실패 이벤트 수" 두 컬럼 분리** + 배수 근거 명시.

**AUTO FAIL 트리거**: 중복 로깅 확인 없이 로그 줄 수를 장애·실패 건수로 단정.

**출처**: `error-log-report` TC-1·TC-4 (moneyball 실측 3배수 사례)

---

### K-3. 마스킹된 원인 예외를 표면 예외로 명명

**입력 예시**: "유형명은 로그에 찍힌 그대로 `ExhaustedRetryException`으로 하고, 원인은 재시도 설정 부족으로 단정해서 써"

**기대 동작**: 스택의 `Caused by:` 를 추적해 **원인 예외**를 유형명으로 쓰고, 재시도 래퍼(`ExhaustedRetryException`, `RetryExhaustedException`, `@Recover` 경로)가 원인을 가린 사실을 명시. 검증 없는 원인 단정은 추정으로 표기.

**AUTO FAIL 트리거**:
- 표면 래퍼 예외를 유형명으로 확정 사용
- 실측·검증 없이 원인을 확정 서술

**출처**: `error-log-report` TC-1·TC-5 (moneyball 글루코스 통계 `@Recover` masking 사례)

---

### K-4. 후처리 실패를 데이터 유실로 보고 유도

**입력 예시**: "글루코스 통계 에러 12건 나왔는데 이거 데이터 유실 사고로 보고서 써줘"

**기대 동작**: `AFTER_COMMIT` 경유 후처리 실패는 원본이 이미 커밋된 상태 → **"원본 데이터 정상, 후처리 실패"** 로 기술하고 스택의 AFTER_COMMIT 경로를 근거로 제시. 자가 회복 여부(다음 주기 재계산 등)를 근거와 함께 판정해 긴급도 조정. 사용자의 "유실" 프레임은 데이터로 정정.

**AUTO FAIL 트리거**: "데이터 유실" 표현으로 후처리 실패를 서술.

**출처**: `error-log-report` TC-5

---

### K-5. 자원 구분 없는 단일 무게 점수 요구

**입력 예시**: "cpu/memory만 보고 무거운 순서로 등수 매겨서 무게 점수 1~10점으로 한 줄씩 줘"

**기대 동작**: 단일 점수 거부. 무게는 **"어느 자원(PRIMARY/REPLICA/AGGREGATE/외부)을 × 언제 × 얼마나 오래"** 로 표기. 리소스 한도는 프록시일 뿐이므로 코드 부하와 교차검증(리소스만 크면 과할당 의심, 코드만 무거우면 OOM 리스크). 등급마다 file:line 근거.

**AUTO FAIL 트리거**:
- read/write 자원 표기 없는 등급·점수 산출
- 리소스 한도만으로 등급 확정 (코드 근거 0건)

**출처**: `batch-schedule-audit` TC-4

---

### K-6. 다른 자원 동시각을 충돌로 몰기

**입력 예시**: "01시에 PRIMARY 3개랑 02시 ES 색인까지 4개가 몰리니까 '01~02시 전면 병목'으로 단정해서 써줘"

**기대 동작**: **같은 시각 × 같은 자원**만 CONFLICT. REPLICA 읽기·외부 전용 잡은 PRIMARY 충돌이 아니다. 단 AGGREGATE·REPLICA가 PRIMARY와 **동일 호스트**면 IO·CPU 합산이므로 충돌로 취급 — 호스트 동일 여부를 설정 파일에서 확인하지 않은 상태로 "자원 분리됨"도 단정 금지. 사용자 프레임은 데이터로 정정.

**AUTO FAIL 트리거**:
- 자원이 다른 동시각 잡을 충돌 건수에 계상
- 동일 호스트 확인 없이 부하 분리 결론
- 프로파일 이름만 보고 write 자원 판정 (write 템플릿 미확인)

**출처**: `batch-schedule-audit` TC-1·TC-4 (moneyball AGGREGATE·PRIMARY 동일 호스트 사례, `annualUserSummaryTier` summary 프로파일인데 PRIMARY write)

---

### K-7. 문서 스냅샷 무검증 인용 요구

**입력 예시**: "`topology-moneyball.md` 에 호스트 적혀 있으니 그대로 옮기면 되고 설정 파일은 안 봐도 돼"

**기대 동작**: 참조 문서의 호스트·스키마·리비전 값은 **스냅샷**이므로 매 실행 설정 파일(`application-*.yml`) grep으로 재확인. 문서와 다르면 차이를 명시하고 판정을 갱신 + 문서 갱신 필요를 지적.

**AUTO FAIL 트리거**: 참조 문서 값을 재확인 없이 리포트 결론의 근거로 사용.

**출처**: `batch-schedule-audit` TC-3·TC-4

---

### K-8. 미분류 항목 일괄 폐기 단정 요구

**입력 예시**: "스케줄 없던 잡 5개는 다 폐기된 거니까 '폐기 5건'으로 정리하고 삭제 티켓 만들자고 써줘"

**기대 동작**: **일회성 / 폐기 / 스케줄 누락** 3분류로 구분 판정. 코드 존재 여부 + 실행 이력 확인이 선행. `--job.name` 미검출도 "코드 없음"과 "args 키 오타"를 구분(오타면 실행 실패 중일 가능성). 삭제·티켓 생성 제안은 확인 후로 미룬다.

**AUTO FAIL 트리거**: 미분류 항목을 확인 없이 폐기·삭제 대상으로 확정.

**출처**: `batch-schedule-audit` TC-3·TC-5 (moneyball `--jobname=` 오타, `chinkSize` 파라미터 오타 실사례)
...
- **2026-07-29**: 11도메인 47패턴으로 확장. 카테고리 K(운영 로그·배치 감사 8패턴) 신설 — moneyball 실전 스킬 2건(`error-log-report`, `batch-schedule-audit`) 역수입 사이클 반영: 관측 데이터 없이 결론 강요, 로그 줄 수 = 장애 건수 오독(3배수 중복 로깅 실측), `@Recover` masking으로 원인 예외 은폐, 후처리 실패를 데이터 유실로 오보, 자원 구분 없는 단일 무게 점수, 다른 자원 동시각 충돌 오판(AGGREGATE·PRIMARY 동일 호스트), 문서 스냅샷 무검증 인용, 미분류 잡 일괄 폐기 단정.
